...
class FavoritesActivity : ComponentActivity() {
    ...
    fun FavoritesScreen(viewModel: FavoritesViewModel) {
        ...
        object FavoritesPreviewData {
            val mockEvents = listOf(
                com.maccs.events.data.model.Event(
                    id = "1",
                    name = "Concierto Rock Urbano",
                    date = 1710493200000L,
                    location = "Madrid",
                    price = 45.0,
                    imageUrl = "",
                    shortDescription = "El mejor rock.",
                ...
                com.maccs.events.data.model.Event(
                    ...
                    price = 60.0,
                    imageUrl = "",
                    shortDescription = "Aprende con Kenji.",
                    longDescription = "Makis y nigiris...",
                    isFavorite = true
                )
            )
        }
...
// --- TODAS LAS VARIANTES DE PREVIEW ---
        ...
        @Preview(showBackground = true, name = "1. Favoritos - Lista Llena")
        @Composable
        fun PreviewFavoritesFull() {
            MaccsEventsTheme(darkTheme = true) {
                FavoritesContent(favorites = FavoritesPreviewData.mockEvents)
            }
        }

        @Preview(showBackground = true, name = "2. Favoritos - Lista Vacía")
        @Composable
        fun PreviewFavoritesEmpty() {
            MaccsEventsTheme(darkTheme = true) {
                FavoritesContent(favorites = emptyList())
            }
        }

        @Preview(showBackground = true, name = "3. Favoritos - OLED Dark Mode")
        @Composable
        fun PreviewFavoritesOLED() {
            MaccsEventsTheme(darkTheme = true) {
                Surface(color = Color.Black, modifier = Modifier.fillMaxSize()) {
                    FavoritesContent(favorites = FavoritesPreviewData.mockEvents.take(1))
                }
            }
        }
    }
